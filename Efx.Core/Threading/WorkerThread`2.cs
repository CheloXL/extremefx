...
// Type: Efx.Core.Threading.WorkerThread`2
...
{
  public abstract class WorkerThread<TArgument, TResult> : IDisposable
  {
    ...
    protected virtual void ThreadFinished(TResult pResult)
    {
    }
    ...
    private void ThreadEntryPoint()
    {
      ...
      TResult pResult = default (TResult);
      try
      {
        pResult = this.DoWork(this._argument);
      }
      catch (ThreadAbortException ex)
      {
      }
      ...
      {
        this.ThreadFinished(pResult);
        this.DisposeThread();
      }
    }

    protected abstract TResult DoWork(TArgument pArgument);
  }
}
